...
import { deletePlayerValuationAction } from './actions';
import { DeleteButton } from '@/components/admin/delete-button';

const PAGE_SIZE = 10;

export default async function PlayerValuationsPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
  ...
  const [totalCount, valuationsDocs] = await Promise.all([
    PlayerValuation.countDocuments({}),
    PlayerValuation.find({})
      .sort({ year: -1, month: -1 })
  ...
  const playerIds = valuationsDocs.map((doc: any) => doc.playerId);
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-emerald-500">Wyceny Zawodników</h1>
        <Link href="/admin/player-valuations/new" className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-md transition-colors">
        ...
        <table className="w-full text-left">
          <thead className="bg-muted text-muted-foreground">
            <tr>
              ...
              <th className="px-6 py-3 font-medium text-sm text-center">Rok / Miesiąc</th>
              ...
              <tr key={item._id.toString()} className="hover:bg-accent/20">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-3">
                    <Link href={`/admin/player-valuations/${item._id}/edit`} className="text-blue-500 hover:text-blue-400">
                      <Edit className="w-5 h-5" />
                    </Link>
                    <form action={deletePlayerValuationAction}>
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          {currentPage > 1 ? (
            <Link href={`/admin/player-valuations?page=${currentPage - 1}`} className="flex items-center gap-1 px-3 py-1.5 bg-card border border-border rounded-md hover:bg-accent hover:text-accent-foreground transition-colors">
