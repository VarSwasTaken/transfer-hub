...
export default async function LeaguesPage({ searchParams }: { searchParams: Promise<{ [key: string]: string | undefined }> }) {
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-3xl font-bold text-emerald-500">Ligi</h1>
        <Link href="/admin/leagues/new" className="flex items-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-md transition-colors">
      ...
      <div className="mb-6">
        <AdminSearchInput placeholder="Szukaj po nazwie lub slugu..." />
      </div>
              ...
              <tr key={item.id} className="hover:bg-accent/20">
                ...
                <td className="px-6 py-4 text-right">
                  <div className="flex justify-end gap-3">
                    <Link href={`/admin/leagues/${item.id}/edit`} className="text-blue-500 hover:text-blue-400">
      ...
      <div className="mt-4 flex items-center justify-between text-sm">
        ...
        <div className="flex gap-2">
          {currentPage > 1 ? (
            <Link href={`/admin/leagues?page=${currentPage - 1}${query ? `&q=${query}` : ''}`} className="flex items-center gap-1 px-3 py-1.5 bg-card border border-border rounded-md hover:bg-accent hover:text-accent-foreground transition-colors">
