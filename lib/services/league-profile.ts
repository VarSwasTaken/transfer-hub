...
export async function getLeagueProfile(leagueId: number): Promise<LeagueProfileResult> {
  ...
  const totalMarketValue = allPlayersInLeague.reduce((sum, player) => sum + (player.marketValue?.toNumber() ?? 0), 0);
  const avgMarketValue = allPlayersInLeague.length > 0 ? totalMarketValue / allPlayersInLeague.length : 0;
...
export async function getLeagueProfileBySlug(slug: string): Promise<LeagueProfileResult> {
  const warnings: string[] = [];
